...
foreach ($row1 as $row) {
  ...
  $w1 =mysqli_query($connection," SELECT name FROM patientregister WHERE id='".$row['name']."'") ;
  $row2=mysqli_fetch_array($w1);
  ?>
  <tr>  
    ...
    <td>
      <a href="ver_contacto.php?id=<?php echo $row['id']; ?>" class="btn bg-blue"><i class="fa fa-eye"></i></a>
      <a href="./donwload.php?file=<?php echo $row['file']; ?>" class="btn bg-blue"><i class="fa fa-download"></i></a>
      <?php
      // Check if there is an image to display
      if (!empty($row['file']) && file_exists($filePath)) {
          ?>
          <a href="../Upload/File/<?php echo $row['file']; ?>" target="_blank" class="btn btn-primary"><i class="fa fa-eye"></i></a>
          <?php
      } else {
      ...
      ?>
      <a href="ResponseContact.php?id=<?php echo $row['id']; ?>"><span class="btn bg-blue"><i class="fa fa-send"></i></span></a>
      <a href="deleted.php?id=<?php echo $row['id']; ?>"><span class="btn btn-danger"><i class="fa fa-trash-o"></i></span></a>
    </td>
  </tr>
  <?php
}
